...
struct DataBackupService {
    ...
    private func allRecords() throws -> [BackupRecord] {
        ...
        output += try context.fetch(FetchDescriptor<ExerciseLocalizationRecord>()).map {
            let issues = ExerciseSupplementCodec.decode($0.commonErrors)
            return record("ExerciseLocalization", $0.id, ["exerciseVersionID": text($0.exerciseVersionID), "localeIdentifier": $0.localeIdentifier, "title": $0.title, "goal": $0.goal, "setup": $0.setup, "steps": json($0.steps), "successCriteria": json($0.successCriteria), "commonErrors": json(issues.problems), "correctiveMeasures": json(issues.measures), "regression": $0.regression, "progression": $0.progression, "homework": $0.homework, "safetyNotes": $0.safetyNotes, "reviewStatus": $0.reviewStatusRawValue])
        }
