...
struct DataBackupService {
    ...
    private func allRecords() throws -> [BackupRecord] {
        ...
        output += try context.fetch(FetchDescriptor<ScheduledSessionRecord>()).map { record("ScheduledSession", $0.id, ["title": $0.title, "startAt": text($0.startAt), "durationMinutes": text($0.durationMinutes), "locationText": $0.locationText, "kind": $0.kindRawValue, "status": $0.statusRawValue, "templateVersionID": text($0.templateVersionID), "calendarEventIdentifier": $0.calendarEventIdentifier]) }
        ...
        output += try context.fetch(FetchDescriptor<TrainingPackageRecord>()).map { record("TrainingPackage", $0.id, ["dogID": text($0.dogID), "name": $0.name, "unitType": $0.unitTypeRawValue, "initialUnits": text($0.initialUnits), "purchasedAt": text($0.purchasedAt), "expiresAt": text($0.expiresAt), "paymentStatus": $0.paymentStatusRawValue, "priceSnapshot": text($0.priceSnapshot), "currencyCode": $0.currencyCode, "isClosed": text($0.isClosed)]) }
